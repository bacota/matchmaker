...
/** What a player is told when a match they accepted into begins.
  *
  * Rendering is separated from deciding who to write to, and is a pure function of its arguments,
  * so that what the mail actually says can be tested without a database, a queue or a match. The
  * deciding half is `GameEngineService.notifyStarted`.
  *
  * Plain text, no HTML alternative. Every line of this is a fact and a link; there is nothing to
  * lay out, and a text part is the one thing every client renders the same way.
  */
object MatchStartedMail {

  /** One line per thing the player needs, in the order they need it: what started, who is in it,
    * whether they are the one holding everyone up, by when, and where to go.
    *
    * `None` when the player has no address, which is the only reason there is nothing to send:
    * the rule that a player with nowhere to write to is skipped lives here rather than in each
    * caller, so that a second kind of notification cannot forget it.
    *
    * @param others      the other players' nicknames, in seat order
    * @param yourTurn    whether the engine has already said this seat moves first
    * @param due         when this player's first turn runs out, if the match is played on a clock
    * @param playUrl     the engine's own link for this match, when it gave one
    */
  def compose(
      sender: String,
      uiBaseUrl: String,
