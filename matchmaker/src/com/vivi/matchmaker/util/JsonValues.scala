...
/** Translation between a JSON value and the plain Scala values the model holds.
  *
  * The model is compiled for Scala.js as well as the JVM and so may not depend on a JSON library — that is why
  * `Result.scores` is a `Map[String, Any]` rather than anything typed. Both places that have to cross that line, the
  * jsonb codec in the persistence layer and the game engine's results callback, use these.
  *
  * Numbers come back as `Double`: JSON keeps no distinction between an integer and a decimal that would survive the
  * round trip anyway.
  */
object JsonValues {
