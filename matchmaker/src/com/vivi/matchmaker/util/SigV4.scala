...
/** Signed HTTP to an AWS service, without an AWS service client.
  ...
  * size, for what is here a single JSON call with no paging, no waiters and no retries. The same
  * trade the game engines make for DynamoDB, in each engine's `Aws.scala`; this is matchmaker's copy
  * rather than a shared one because an engine deliberately shares no code with matchmaker.
  */
class SigV4(
