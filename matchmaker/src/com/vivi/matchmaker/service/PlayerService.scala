...
class PlayerService(sessionPool: SessionPool) {
    ...
    /** Players whose nickname begins with `prefix`, for the search box.
      *
      * Case sensitive, because a nickname is: two players may be registered as "Ash" and "ash", the database's unique
      * index treats them as two names, and a search that folded case would offer them as one another's results. It also
      * means the prefix somebody types is the prefix that is matched, with no rule to explain.
      *
