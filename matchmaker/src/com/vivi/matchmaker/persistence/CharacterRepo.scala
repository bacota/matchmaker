...
class CharacterRepo[T](session: Session[IO])(using codec: TextCodec[T]) {
    ...
    /** Reads a character together with its owning player and its game, in a single query, by joining the character,
      * player, character_game, and game tables. Returns None both when no character with this id exists and when it has
      * no owning player (since it then has no matching row in this join).
      */
    ...
    /** As `readWithOwnerAndGame`, but locking the character row until the enclosing transaction ends, so that a caller
      * which reads, authorizes and then writes cannot have the row change underneath it in between.
      */
    ...
    /** Reads a character together with its game, in a single query, joining the character and game tables. Unlike
      * readWithOwnerAndGame, this does not require the character to have an owning player.
      */
    ...
    /** Every character this player has in this game.
      *
      * Ordered by name so the list is stable between calls: an unordered query may return rows in whatever order the
      * planner likes, which shows up as a select that reshuffles itself.
      */
    def listForPlayerAndGame(playerId: PlayerId, gameId: GameId): IO[List[Character[T]]] =
        session
            .execute(selectCharactersForPlayerAndGame)((playerId, gameId))
            .map(_.map { case (id, name, state, description) =>
                Character(id, gameId, name, description, state, Some(playerId))
            })
