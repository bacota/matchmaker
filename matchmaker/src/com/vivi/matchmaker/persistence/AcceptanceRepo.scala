...
class AcceptanceRepo(session: Session[IO]) {
            ...
            case GameType.Character =>
                val cid = characterIdValue.getOrElse(
                  throw new IllegalStateException(
                    s"acceptance ($challengeId, $playerId) is game_type 'C' but has no character_acceptance row"
                  )
                )
    ...
    /** The roles already claimed by the acceptances of one challenge.
      *
      * Two questions are answered from this and nothing else: whether a role a player is asking for is still free, and
      * whether every required role of the game has been taken -- which is what a start waits for. Cheaper than
      * [[listForChallenge]], which joins in players and characters that neither question needs.
      */
    ...
    /** Reads, in one join query, everything needed to authorize deleting an acceptance: the challenge it belongs to,
      * the accepting player, and the challenger (the player who owns the challenge).
      */
    def readWithChallengeAndPlayers(
        gameId: GameId,
        challengeId: ChallengeId,
        playerId: PlayerId
        ...
        session
            ...
            .map(_.map {
                case (
                    ...
                    val acceptor =
                        Player(playerId, acceptorNickname, acceptorIsAdmin, acceptorExternalId, acceptorEmail)
    ...
    /** Every acceptance this player has outstanding, each with the two facts about its challenge that decide what the
      * UI can offer on it: who may start it, and whether it is ready to be.
      *
      * "Outstanding" means the player has not backed out and the challenge has not been started. The acceptance rows of
      * a started challenge are kept — they are the roster the match was made from, and the challenge that owns them is
      * kept too — but they are no longer anything the player can act on, so they are excluded here rather than
      * lingering in the UI's list of things to back out of. This is what "what have I said yes to?" means — the UI
      * needs it to offer backing out without first knowing which challenge to look under. Scoped by player_id alone
      * rather than the full composite key deliberately: a player may have acceptances across many games, and listing
    ...
    def listForPlayer(playerId: PlayerId): IO[List[PendingAcceptance]] =
        session
            .execute(selectAcceptancesForPlayer)(playerId)
            .map(_.map { case (challenge, gt, gameId, roleId, characterIdValue, challenger, ready) =>
                PendingAcceptance(
    ...
    def create(a: Acceptance): IO[Acceptance] =
        a match {
            case ca: CharacterAcceptance =>
                session
                    .execute(insertCharacterAcceptance)(
                      (a.challengeId, a.playerId, a.gameId, a.gameRoleId, ca.characterId)
                    )
                    .as(a)
            case _: PlainAcceptance =>
                session
                    .execute(insertAcceptance)((a.challengeId, a.playerId, GameType.Plain, a.gameId, a.gameRoleId))
                    .as(a)
        }
    ...
    def read(gameId: GameId, challengeId: ChallengeId, playerId: PlayerId): IO[Option[Acceptance]] =
        session
            ...
            .map(_.map { case (gt, roleId, characterIdValue) =>
