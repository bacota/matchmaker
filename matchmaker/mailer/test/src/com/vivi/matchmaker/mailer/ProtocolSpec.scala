...
/** The one thing this module's tests depend on matchmaker for: that the mail matchmaker writes is the mail this
  * function reads.
  *
  * `MailMessage` is stated twice on purpose — see the class comment on this module's copy — so this reads each side's
  * JSON with the other side's class. A field renamed on either side fails here, rather than as messages that go round
  * the queue three times and die in the dead-letter queue.
  */
