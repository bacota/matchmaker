...
/** Drains the mail queue into SES.
  *
  * Handler string: `com.vivi.matchmaker.mailer.Handler::handleRequest`, with an SQS event source
  * mapping in front of it — which is the poller: Lambda long-polls the queue and hands this
  * batches of messages.
  *
  * This function knows nothing about games, matches or players. It receives four strings and
  * sends them, which is what lets every later kind of notification reuse it untouched.
  */
...
object Handler {
  ...
  /** The messages in an SQS event.
    *
    * A record whose body is not a `MailMessage` is dropped rather than failed. Redelivering it
    * cannot help — it will not parse the next time either — so the only outcomes are to drop it
    * now or to have the queue retry it to its dead-letter queue three attempts later. Dropping is
    * the quieter of the two, and the parse failure is logged where the body can be read.
    */
  ...
  /** Built once per container. The region is the one the function runs in; the credentials are
    * the execution role's, from the variables the runtime sets.
    */
