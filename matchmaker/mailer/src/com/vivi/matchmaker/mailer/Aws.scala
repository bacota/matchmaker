...
/** The credentials to sign with, from the standard variables the Lambda runtime populates from the execution role.
  * `None` anywhere else, which is what makes a local run need no AWS at all.
  */
...
/** Signed HTTP to an AWS service, without an AWS service client.
  *
  * The SDK's *signer*, and the JDK's own HTTP client. A service client would bring Netty and Apache HttpClient — some 8
  * MB — into a Lambda jar whose cold start is proportional to its size, for what is here one JSON call per mail with no
  * paging and no waiters.
  *
  * The third copy of this in the repository, after matchmaker's `util.SigV4` and each engine's `Aws.scala`, and copied
  * for the same reason the message type above is: this module is deployed on its own and depends on nothing. It is some
  * ninety lines of a signer being handed the four properties it needs, and sharing it would cost far more than
  * restating it.
  */
class SigV4(
    ...
    /** POSTs `body` to `url`, signed for `service` when there are credentials to sign with.
      *
      * Unsigned when there are none. That cannot weaken a deployed call: AWS rejects an unsigned request rather than
      * accepting it, so the failure is loud and immediate — and it is what lets a developer point this at a local fake
