...
/** Applies pending Flyway migrations to the local test database before any spec runs, so specs never depend on
  * migrations having been run manually beforehand.
  */
