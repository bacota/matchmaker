            ...
            TestSession.resource
                .use { session =>
                    ...
                    for {
                        ...
                        createdPlayer <-
                            if (withPlayer) playerRepo.create(player).map(p => Some(p.playerId)) else IO.pure(None)
            ...
            TestSession.resource
                .use { session =>
                    ...
                    for {
                        ...
                        character <- IO.pure(
                          Generators.genCharacter(createdGame.gameId, Some(createdPlayer.playerId)).sample.get
                        )
                        created <- characterRepo.create(character)
