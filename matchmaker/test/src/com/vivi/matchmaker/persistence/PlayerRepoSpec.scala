    ...
    forAll(Generators.genPlayer) { player =>
      TestSession.resource
        .use { session =>
          val repo = new PlayerRepo(session)
          for {
            created <- repo.create(player.copy(email = Some(s"original-${java.util.UUID.randomUUID()}@example.com")))
            // A rename, with a stale address in the same object — exactly the shape of the mistake.
            _ <- repo.update(created.copy(nickname = s"renamed-${java.util.UUID.randomUUID()}", email = None))
            found <- repo.read(created.playerId)
    ...
    forAll(Generators.genPlayer) { player =>
      TestSession.resource
        .use { session =>
          val repo = new PlayerRepo(session)
          for {
            created <- repo.create(player.copy(email = Some(s"gone-${java.util.UUID.randomUUID()}@example.com")))
