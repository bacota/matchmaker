...
/** Which notifier an environment gets.
  *
  * The sending itself is not tested here — that would need a queue — but the choice in front of it is worth pinning
  * down, because getting it wrong is silent in both directions: an environment that should send mail and quietly does
  * not, or one that has no queue and fails every start's last step trying to reach one.
  */
class SqsNotifierSpec extends FunSuite {
    ...
    test("an environment naming a queue gets a notifier that will send to it") {
        val notifier = SqsNotifier.fromEnvironment(
          env(
            "MAIL_QUEUE_URL" -> "https://sqs.eu-west-2.amazonaws.com/1/matchmaker-dev-mail",
