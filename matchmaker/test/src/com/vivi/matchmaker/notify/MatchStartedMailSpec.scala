...
/** What the mail actually says.
  *
  * Pure, so it is tested here rather than through a started match: composing the text needs no
  * database, and the rules worth pinning down — a player with no address gets nothing, a
  * challenge with no message falls back to the game's name, a deadline is only quoted when there
  * is one — are all decisions this function makes on its own.
  */
class MatchStartedMailSpec extends FunSuite {

  private val game =
    Game(GameId(1), GameType.Plain, "Tic-Tac-Toe", "a game", "https://engine/games", active = true, Seq.empty, Seq.empty, "ttt")
