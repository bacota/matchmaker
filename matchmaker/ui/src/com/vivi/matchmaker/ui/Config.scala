...
/** Where this build of the UI points: the API, and the Cognito pool in front of it.
  *
  * Read from a `window.matchmakerConfig` object rather than compiled in, so that the same `main.js` can be served
  * against dev or prod by changing one script tag. Every value here is public — the client id is a query parameter of
  * the authorize URL — so none of this is a secret being handed to the browser.
  *
  * The values come from the terraform outputs of the matching environment: `api_endpoint`, `hosted_login_url` and
  * `user_pool_client_id`.
  */
case class Config(
    apiEndpoint: String,
    ...
    /** True in the local development mode, where there is no Cognito and the caller's identity is asserted rather than
      * proved. See `Config.HeaderAuth`.
      */
...
object Config {
    ...
    /** Development only: the identity is sent in `X-External-Id` and believed. It exists to click through the UI
      * against `LocalServer` with no AWS involved.
      *
      * It is worth being blunt about what this is: **authentication turned off**. Anyone who can reach the API can be
      * anyone. It is usable only because `LocalServer` binds to loopback and the deployed function does not accept it —
      * `Handler` defaults to `gateway`, and the terraform sets `AUTH_MODE=gateway` explicitly.
      */
    val HeaderAuth = "header"

    /** Fails loudly and immediately on a missing field. A UI that started up and then failed every request with an
      * opaque error would be far harder to diagnose than one that does not start.
      */
    lazy val current: Config = {
        ...
        if (mode != CognitoAuth && mode != HeaderAuth)
            throw new IllegalStateException(
              s"matchmakerConfig.authMode is '$mode'; expected '$CognitoAuth' or '$HeaderAuth'"
            )
        ...
        Config(
          ...
          region = optional("region")
              ...
              .getOrElse(
                if (mode == HeaderAuth) "" else throw new IllegalStateException("matchmakerConfig.region is not set")
              ),
    ...
    /** The region out of `https://<prefix>.auth.<region>.amazoncognito.com`.
      *
      * Only the Cognito-provided domain has this shape. A pool behind a custom domain does not, so this returns `None`
      * and `region` has to be configured explicitly.
      */
