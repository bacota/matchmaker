...
/** The bits of the Cognito user pools API this UI calls directly.
  ...
  *
  * These are *unauthenticated* operations — `InitiateAuth`, `RespondToAuthChallenge` and
  * `RevokeToken` are how a caller with no credentials obtains some, so they are not SigV4 signed.
...
object CognitoIdp {
  ...
  /** What a successful authentication returns. `refreshToken` is absent when refreshing: that
    ...
    */
  case class Tokens(idToken: String, refreshToken: Option[String])
