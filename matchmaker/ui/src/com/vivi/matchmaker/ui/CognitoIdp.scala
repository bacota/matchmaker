...
/** The bits of the Cognito user pools API this UI calls directly.
  *
  * Not an SDK: the three operations used here are plain JSON POSTs to one endpoint, and pulling in the AWS SDK to make
  * them would be far more machinery than the calls are worth.
  *
  * None of them are SigV4 signed. `InitiateAuth`, `RespondToAuthChallenge` and `RevokeToken` are how a caller with no
  * credentials obtains some; the account operations — `ChangePassword`, `UpdateUserAttributes`, `VerifyUserAttribute` —
  * authorize with the user's own access token instead, which is why they need one rather than the ID token every API
  * call carries. The app client is public (`generate_secret = false` in the terraform), so there is no `SECRET_HASH` to
  * compute either, which is what makes this reachable from a browser at all.
  *
  * The wire format is AWS's JSON 1.1 protocol: the operation is named in `X-Amz-Target` rather than in the path, and a
  * failure is an HTTP 400 whose body carries the exception type.
  */
object CognitoIdp {

    private def endpoint: String = s"https://cognito-idp.${Config.current.region}.amazonaws.com/"

    /** What a successful authentication returns. `refreshToken` is absent when refreshing: that grant reuses the
      * refresh token already held rather than issuing a new one.
      *
      * `accessToken` is the one the *user pools API* accepts, and is what the account operations below are authorized
      * with — an ID token is not accepted there, whatever its claims say. It is an `Option` because a response that
      * omitted it would still be a usable sign-in; what depends on it is the account menu, not the session.
      */
    ...
    /** Cognito wants something more from the caller before it will issue tokens.
      *
      * `session` is opaque and single-use — it must be echoed back on the response, and a new one comes back if that
      * response is itself challenged.
      *
      * `parameters` carries whatever the particular challenge needs the user to be told. For `EMAIL_OTP` that is
      * `CODE_DELIVERY_DESTINATION`, the masked address the code went to, which is worth showing so the user knows which
      * mailbox to open.
      */
    ...
    /** A rejection from Cognito, keeping the `__type` so callers can tell "wrong password" from "this user is not
      * confirmed" — which need entirely different things said to the user.
      *
      * `message` is Cognito's own wording. It is shown as-is: these strings are written to be read by end users, and
      * paraphrasing them here would mean maintaining a second copy of Cognito's error catalogue.
      */
    case class IdpError(errorType: String, message: String) extends Exception(s"$errorType: $message")

    /** The request never got an answer, or got one that was not JSON — a dropped connection, a 5xx, a proxy error page.
      * Distinct from `IdpError` because it says nothing about the credentials and is worth retrying.
      */
    ...
    /** Starts a sign-in with the choice-based flow, naming the factor to try first.
      *
      * `USER_AUTH` is the flow that lets the *client* decide what to ask for, which is the whole reason this page
      * exists rather than a redirect to managed login: managed login enables every factor the pool allows and puts the
      * passwordless one forward, and there is no setting that reorders it.
      *
      * Passing `PASSWORD` here alongside the password itself resolves in one round trip when the password is right.
      * Passing `EMAIL_OTP` (with no password) comes back as a challenge, having mailed a code.
      */
    ...
    /** Exchanges a refresh token for a fresh ID token.
      *
      * Deliberately not the hosted UI's `/oauth2/token` endpoint. A refresh token minted by `InitiateAuth` was never
      * issued against an OAuth grant, and the token endpoint will not redeem it — which fails silently at the first
      * expiry rather than at sign-in, so it is worth being explicit about.
      */
    ...
    /** Revokes a refresh token, and with it every access token issued from the same sign-in.
      *
      * This is what makes signing out mean something. Without it the tokens in this tab are dropped but remain valid
      * until they expire, so anything that captured one keeps working.
      */
    ...
    /** Changes the signed-in user's password, given the current one.
      *
      * Cognito checks `PreviousPassword` itself, which is what makes this safe to offer to a session that may have been
      * left open: knowing the old password is required, so a stolen token alone cannot lock the owner out.
      */
    ...
    /** Asks Cognito to change the signed-in user's email address.
      *
      * The change is not complete when this returns. The pool auto-verifies email, so Cognito mails a code to the *new*
      * address and leaves the attribute unverified until `verifyEmail` quotes it back — which is the point: it proves
      * the address before it becomes the thing the account signs in with. The masked destination comes back so the user
      * knows which mailbox to open.
      */
    ...
    /** Reads an error body. Cognito names the exception in `__type`, sometimes as a bare name and sometimes qualified
      * with a namespace (`com.amazonaws...#NotAuthorizedException`); only the last segment is meaningful, so callers
      * can match on it without knowing which form arrived.
      */
