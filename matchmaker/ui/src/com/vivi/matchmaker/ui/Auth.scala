...
/** Sign-in against the Cognito user pools API, with the hosted pages kept for sign-up and reset.
  ...
  *
  *   - *Signing in* is done here, by `SignIn` driving `CognitoIdp.initiateUserAuth`. That is what buys the thing
  *     managed login will not do: with `EMAIL_OTP` enabled on the pool, managed login puts the emailed code forward and
  *     there is no setting that reorders it. Naming `PREFERRED_CHALLENGE` ourselves asks for the password first and
  *     leaves the code as a second option.
  *   - *Signing up* and *resetting a password* still redirect to the hosted pages (`hostedSignUp`,
  *     `hostedForgotPassword`), which come back through the authorization code grant below. Those flows are several
  *     screens each and Cognito already has them.
  *
  * The cost of the first half is that the password is typed into this application rather than into Cognito's own page,
  * so script running on this origin could read it — not merely steal a session, as before. What holds that line is that
  * this page loads no third-party script; a CSP on the distribution is what would keep it held.
  *
  * What either route ends with is an ID token, which every API call carries and which API Gateway's JWT authorizer
  * verifies.
  *
  * The tokens are kept in `sessionStorage`, not `localStorage`: they are cleared when the tab closes, and are not
  * shared with other tabs. Neither is proof against XSS — script running on this origin can read either — so the
  * protections that remain are that this page loads no third-party script and that the session dies with the tab.
  *
  * The refresh token is stored alongside the ID token so a session outlives the ID token's hour (see `freshIdToken`).
  * That is a deliberate trade: it lengthens what a successful XSS could do with what it steals, in exchange for not
  * signing players out mid-game.
  */
object Auth {
    ...
    /** The current ID token, if there is one that has not expired.
      *
      * Expiry is checked here as well as by the gateway, so that the UI shows a sign-in button rather than firing a
    ...
    /** The signed-in player's email address, from the ID token's `email` claim.
      *
      * Read from the stored token whether or not it has expired, unlike `idToken`: this is a label to show the player,
      * not a credential to send, and an expired token still names the account it was issued for. `None` when there is
      * no session, or when the token carries no `email` claim — a pool configured without the attribute, or a scope
      * that did not ask for it.
      *
      * Note that it is the token's word, not Cognito's current one: a change made in this tab is not in the token until
      * the session refreshes, which is why `Account` updates what it shows from the address the player just confirmed.
      * It is also why matchmaker's own copy of the address is reconciled from this claim only at sign-in, when the
      * token is new enough for the claim to be current — see `Store.syncEmail`.
      */
    ...
    /** A session exists when there is a usable ID token, or a refresh token that can obtain one. Used for rendering;
      * `freshIdToken` is what actually establishes whether the refresh works.
      */
    ...
    /** The current access token, if there is one that has not expired.
      *
      * Distinct from the ID token, and not interchangeable with it: the API carries the ID token, and the Cognito user
      * pools API accepts only this one for the account operations. It is stored under its own key, and a session that
      * predates that key simply has none — which is what `freshAccessToken` refreshes to obtain.
      */
    private def accessToken: Option[String] =
        Option(dom.window.sessionStorage.getItem(AccessKey)).filter(unexpired)

    /** At most one refresh is in flight: several requests hitting an expired token at once must redeem the refresh
      * token once between them, not race to redeem it each.
      */
    ...
    /** An ID token that is good to send, refreshing first if the current one has run out.
      *
      * `None` means there is no session left — either nothing was stored, or the refresh token was rejected, in which
      * case the stored session has already been cleared. A failed future means the refresh could not be attempted
      * (Cognito unreachable, a 5xx); the session survives and the caller's request fails instead.
      */
    ...
    /** An access token that is good to send to the Cognito user pools API, refreshing first if there is not one — which
      * includes a session established before access tokens were stored at all, where the ID token is still perfectly
      * good and this is simply absent.
      *
      * `None` means the account operations cannot be offered: there is nothing left to refresh with, or the refresh
    ...
    /** Exchanges the refresh token for a new ID token.
      *
      * Through `REFRESH_TOKEN_AUTH` on the user pools API, not the hosted UI's `/oauth2/token`: a refresh token minted
      * by `InitiateAuth` was not issued against an OAuth grant and the token endpoint will not redeem it. Tokens
      * obtained from the hosted sign-up flow *would* refresh at either, so using the IdP call for both is what keeps
      * one code path here.
      *
      * `None`, with the stored session cleared, means the session is genuinely over (see `endsSession`). A failed
      * future means this attempt did not get through and the session is still intact. Cognito does not return a new
      * refresh token here; one is stored if it does.
      */
    ...
    /** Whether a failed refresh means the session itself is over, rather than that this attempt did not get through.
      *
      * The distinction matters: clearing the session on a dropped connection or a Cognito 5xx would sign a player out
      * mid-game over something that would have worked a second later. Those failures are left to propagate, so the call
      * that triggered the refresh fails and can be retried with the session intact.
      */
    ...
    /** Sends the browser to one of the hosted pages. Does not return: the page navigates away.
      ...
      * `redirectUri` with an authorization code, so both need PKCE set up first, and both land in `completeSignIn` on
      * the way back: a player who has just confirmed a sign-up or set a new password arrives already signed in, which
      * is the point of sending them to a page that can do that rather than back to the sign-in form.
      */
    ...
    /** Ends the session, here and at Cognito.
      *
      * Revoking is what makes this more than clearing storage: the ID token stays valid until it expires whatever this
      * page does, but revoking the refresh token stops the session being extended past that. It is fire-and-forget — a
      * revoke that does not get through must not leave the user still apparently signed in on a page they asked to
      * leave — so the local session is cleared first and the failure is swallowed.
      *
      * There is no redirect to the hosted `/logout` any more. That existed to end the *hosted UI's* own browser
      * session, and signing in here never creates one. It is still worth a thought for a player who used the hosted
      * sign-up page: that visit did leave a Cognito session cookie, and it is what `hostedSignUp` would silently reuse.
      * It expires on its own, and reaching it requires clicking sign-up again, so it is left alone rather than
      * redirecting every sign-out away from the page.
      */
    def signOut(): Unit = {
    ...
    /** Completes a sign-in if this page load is Cognito's redirect back.
      *
      * Returns `Some` only when a code was present, so the caller can tell "just signed in" from "ordinary page load".
      * Errors from Cognito arrive as `?error=` on the same redirect and are surfaced rather than ignored — an unusable
      * login that reports nothing is the worst outcome.
      */
                ...
                Option(params.get("code")).map { code =>
                    ...
                    if (returnedState.isEmpty || returnedState != expectedState)
                        ...
                        verifier match {
                            case None =>
                                Future.failed(
                                  new IllegalStateException("no PKCE verifier for this callback; start sign-in again")
                                )
    ...
    /** Redeems the authorization code for tokens, presenting the PKCE verifier.
      *
      * No client secret and so no `Authorization` header: this is a public client, which is exactly the case PKCE
      * exists for.
      */
    ...
    /** Stores whichever tokens a token response carried. A refresh grant returns no refresh token of its own, so the
      * one already stored is left in place rather than cleared.
      */
    private def store(body: String): Unit = {
        ...
        Try(ujson.read(body)("refresh_token").str).foreach(token =>
            dom.window.sessionStorage.setItem(RefreshKey, token)
        )
    }
    ...
    /** A non-200 from the token endpoint, keeping the status and body so the caller can tell a rejected token from an
      * unreachable Cognito.
      */
    private case class TokenError(status: Int, body: String) extends Exception(s"token request failed ($status): $body")

    private case class MalformedTokenResponse() extends Exception("token response carried no id_token")

    /** True when the token's `exp` is still in the future, with a small margin so that a token about to expire is not
      * sent on a request that will outlive it.
      */
    ...
    /** One string claim of a token, or `None` if the token cannot be read or does not carry it.
      *
      * Non-string claims are `None` rather than stringified: every claim this reads is a string, and rendering
      * `["a","b"]` into a sentence is worse than rendering nothing.
      */
    ...
    /** The payload of a JWT, unverified.
      *
      * The signature is deliberately not checked, here or in `unexpired`. Nothing on this page could act on the result
      * — the gateway is what decides whether a token is good, and it does verify the signature. This only decides
      * whether to bother asking, and what to call the player while it does.
      */
