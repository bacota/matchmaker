...
/** The sign-in form, and the challenge run behind it.
  *
  * Cognito's `USER_AUTH` flow is a conversation rather than a single call: a first request names the factor to try, and
  * the answer is either tokens or another challenge carrying a `Session` to echo back. This holds the position in that
  * conversation.
  *
  * The reason it exists at all is ordering. The pool allows `PASSWORD` and `EMAIL_OTP` as first factors, and managed
  * login decides between them itself — putting the emailed code first, with no setting to change it. Asking for
  * `PREFERRED_CHALLENGE = PASSWORD` here is what makes the password the default and the code an alternative the user
  * can take.
  */
object SignIn {
    ...
    enum Stage {
        ...
        /** Cognito sent a code and wants it back. `challenge` is its own name for the challenge, which decides the
          * field the answer goes in — see `codeResponseKeys`.
          */
        case Code(challenge: String, username: String, session: String, deliveredTo: Option[String])

        /** A first sign-in on a temporary password. Reached by the administrator created in the terraform when
          * `admin_initial_password` was left unset, and by anyone Cognito has had a password reset forced on.
          */
    ...
    /** The field a delivered code goes back in, per challenge.
      *
      * Only `EMAIL_OTP` can arrive with the pool as it is configured — there is no MFA and no phone number. The rest
      * are here because they are the same screen with a different key, and a challenge this does not recognise is a
      * dead end for the user in front of it.
      */
    ...
    /** True while a request is in flight. Disables the buttons, so that a slow answer does not become two sign-in
      * attempts — the second of which would arrive with a spent `Session`.
      */
    private val busy: Var[Boolean] = Var(false)

    /** Shown above the form. Not `Store.error`: this belongs to the form and should clear when the user tries again,
      * rather than joining the application-wide errors in the header.
      */
    ...
    /** The ordinary path: email and password, one round trip when the password is right.
      *
      * The password goes in `AuthParameters` alongside the preference, so a correct one comes back as tokens rather
      * than as a `PASSWORD` challenge to answer separately.
      */
    ...
    /** Runs one step and advances, or reports why it did not.
      *
      * `username` is threaded through because Cognito does not echo it and every subsequent response has to carry it.
      */
        ...
        case CognitoIdp.AuthOutcome.Challenged(challenge) =>
            challenge.name match {
                ...
                case other =>
                    problem.set(
                      Some(
                        s"This account needs a sign-in step this page does not support ($other). Try resetting your password."
                      )
                    )
            }
    }
    ...
    /** Back to an empty form. Called on success so that the password does not sit in a `Var` for the rest of the
      * session, and on "start again" so a half-finished challenge is not resumed with a spent session.
      */
    ...
    /** What to put in front of the user for a failure.
      *
      * Cognito's own wording is used for anything not named here: those strings are written to be read by end users,
      * and a second copy of its error catalogue is not worth maintaining. The exceptions are the few where the right
      * thing to say includes what to do next.
      */
    ...
    private def credentials: HtmlElement =
        form(
          ...
          label(
            "Email",
            input(tpe := "email", autoComplete := "username", value <-- email, onInput.mapToValue --> email)
          ),
          label(
            "Password",
            input(
