...
/** Reading what Cognito answers with.
  *
  * Worth testing because the shapes are not interchangeable and the failure modes are quiet: a challenge misread as an
  * authentication stores no token and reports nothing, and a `Session` dropped on the floor makes the *next* request
  * fail rather than this one.
  */
class CognitoIdpSpec extends FunSuite {
    ...
    test("a completed authentication yields both tokens") {
        ...
        assertEquals(
          ...
          Success(
            CognitoIdp.AuthOutcome.Authenticated(CognitoIdp.Tokens("id.jwt", Some("refresh"), Some("access.jwt")))
          )
        )
    }
