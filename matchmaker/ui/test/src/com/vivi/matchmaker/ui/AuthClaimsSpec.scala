...
/** Reading a claim out of an ID token.
  *
  * Worth testing because the encoding is the part that is easy to get wrong and quiet when it is: a JWT payload is
  * base64url with the padding dropped, and a decoder that assumes standard base64 fails only on the tokens whose
  * payload length happens to need it — which is most of them, but not the short one someone tries by hand.
  */
class AuthClaimsSpec extends FunSuite {

    /** A token as Cognito issues one: three dot-separated parts, of which only the payload is read. The signature is
      * not checked here and is not checked in `Auth` either — the gateway is what decides whether a token is good.
      */
