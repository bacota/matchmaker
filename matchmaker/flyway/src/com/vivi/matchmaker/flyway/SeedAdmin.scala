...
/** Ensures the administrator's player row exists for an already-created Cognito user.
  *
  * Terraform creates the user; it cannot create the row, because a player lives in a database inside the VPC and
  * terraform has no resource for inserting one. So the two halves are joined here, by `sub`: deploy.sh applies
  * terraform, reads the `admin_external_id` output, and runs this. It lives in the flyway module rather than the
  * service layer because it needs exactly what that module already has — a JDBC driver and nothing else — and because
  * it runs in the same place, from an operator's machine with a route into the VPC.
  *
  * Idempotent, and run on every deploy: an existing row is granted admin rather than duplicated, and a run that changes
  * nothing says so.
  *
  * Configuration is read from environment variables: FLYWAY_URL jdbc:postgresql://host:port/database (default:
  * jdbc:postgresql://localhost:5432/matchmaker) FLYWAY_USER database user (default: matchmaker) FLYWAY_PASSWORD
  * database password (default: empty) ADMIN_EXTERNAL_ID the Cognito `sub` of the admin user (required) ADMIN_NICKNAME
  * nickname for a newly created row (default: admin)
  *
...
object SeedAdmin {
    ...
    /** Returns a description of what it did, for the deploy log. Separated from `main` so it can be driven against a
      * test database without going through environment variables.
      */
