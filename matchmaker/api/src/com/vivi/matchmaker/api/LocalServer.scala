...
/** Runs the API on a local port, against a local Postgres, with no AWS involved.
  *
  * Everything below API Gateway is the deployed code: the same `Router`, the same services, the same SQL. Only the two
  * things API Gateway and Lambda provide are replaced — the transport, by the JDK's own HTTP server, and the caller's
  * identity, by the `Authenticator` chosen here.
  *
  * What it therefore does not check is the deployment itself: the handler string, the payload format, cold-start
  * behaviour. `RouterSpec` and `ApiGatewaySpec` cover the payload translation; the rest only a real deploy will tell
  * you about.
  *
  * {{{
...
object LocalServer {
    ...
    /** Origins the browser UI may call this from.
      *
      * The UI is served by a separate static server, so every call it makes is cross-origin. Only loopback origins are
      * allowed and only ones listed here: this process trusts an unverified header for identity, so anything that can
      * call it can be anyone, and a permissive `*` would hand that to any page the developer happens to have open.
      */
    ...
    /** Local runs trust the header. `gateway` is not offered here on purpose: it reads claims that only API Gateway's
      * authorizer can put there, so locally it would authenticate nobody. Verifying a real dev-pool token in-process is
      * `Authenticator.VerifiedToken`, which does not exist yet.
      */
    ...
    /** Credentials come straight from the environment here, where the Lambda reads them from Secrets Manager. The
      * defaults are the local development database the tests already use, so that a checkout with Postgres running
    ...
    private class Dispatcher(services: Services[String], authenticator: Authenticator) extends HttpHandler {
        ...
        /** Echoes back the one origin that matched rather than `*`, which is both what a credentialed request requires
          * and a smaller thing to get wrong. Set before the status line, since the JDK server writes headers when
          * `sendResponseHeaders` is called.
          */
