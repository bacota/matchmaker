...
/** Just enough of the API Gateway HTTP API payload format v2 to route a request and answer it.
  *
  * The event is picked apart with ujson rather than decoded into a case class: the real payload carries a large
  * `requestContext` (authorizer, identity, http, ...) that would have to be modelled in full, and only a few fields of
  * it matter here.
  */
object ApiGateway {

    /** A decoded request. Header names are lowercased on the way in, because payload v2 lowercases them and a
      * case-sensitive lookup for "X-External-Id" would silently never match.
      */
    case class Request(
        method: String,
        path: String,
        ...
        /** A claim from the token the gateway's JWT authorizer already verified.
          *
          * Empty when there is no authorizer in front of the function — locally, or if the route were ever left
          * unauthenticated — so a caller of this must decide what an absent claim means rather than assuming the
    ...
    /** Header carrying the caller's identity in the local development mode, where there is no gateway to verify a
      * token. Deployed, the identity is the `sub` claim instead — see `Authenticator`.
      */
