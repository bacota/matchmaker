...
object Errors {
    ...
    /** Turns a failure into a response.
      *
      * Anything that is not a `ServiceError` is an infrastructure failure — a dropped connection, a bug — and its
      * message could disclose internals, so only a generic message is returned to the caller. The real exception is
      * written to stderr here (rather than left to the caller to log, which nothing was actually doing) so it lands in
      * CloudWatch either way.
      *
      * `where` names the request that failed. Without it a stack trace in CloudWatch cannot be matched to the call that
      * produced it, which is most of what makes a 500 diagnosable.
      */
    ...
    /** Writes a failure to stderr, with its cause chain, as one record.
      *
      * Every 5xx this service produces goes through here. `printStackTrace` alone was not enough: it drops the request
      * that failed, and the runtime interleaves its lines with other output, so a trace could not reliably be read back
      * as a unit.
      */
    ...
    /** No verified `sub` reached the function. Deployed, the gateway's authorizer answers 401 long before this, so
      * seeing it means a request arrived without passing one.
      */
