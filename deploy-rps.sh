#!/usr/bin/env bash
#
# Builds and deploys the rock-paper-scissors game engine, without redeploying matchmaker.
#
#   ./deploy-rps.sh dev
#   ./deploy-rps.sh dev --yes         # skip the confirmation between plan and apply
#   ./deploy-rps.sh dev --full        # plan the whole configuration, not just the engine
#   ./deploy-rps.sh dev --skip-build  # the jar must already exist
#
...
# anywhere with AWS credentials. It is independent of deploy-tictactoe.sh too: the two engines
# share nothing but the user pool they authenticate players against.
#
...
readonly jar="out/engines/rps/assembly.dest/out.jar"
...
# ---------------------------------------------------------------------------
#
# deploy_rps defaults to false, so without it this would plan, apply and report success
...
if ! grep -Eq '^[[:space:]]*deploy_rps[[:space:]]*=[[:space:]]*true' "$settings" 2>/dev/null; then
  echo "the rock-paper-scissors engine is not enabled for '$env'" >&2
  echo >&2
  echo "add this to $settings:" >&2
  echo >&2
  echo '    // The bundled rock-paper-scissors engine: a test fixture for simultaneous turns.' >&2
  echo '    deploy_rps = true' >&2
  exit 1
fi
...
if [ "$env" = "prod" ]; then
  ...
  printf '\n\033[1mrock-paper-scissors is a test fixture; deploy it to prod anyway? [y/N] \033[0m'
  read -r reply
  case "$reply" in
    y | Y | yes | YES) ;;
    *)
...
# wire format, which is exactly the mistake a deploy would otherwise ship — and this engine's
# copy of it covers the parts of that format only a game with no turn order uses.
...
if [ "$skip_build" = true ]; then
  ...
  mill -j 4 --ticker false engines.rps.assembly
fi
...
plan_file=$(mktemp "${TMPDIR:-/tmp}/rps-$env-XXXXXX.tfplan")
...
# engine a callback came from; the terraform files it under "rps".
#
...
create_game_url=$(output rps_create_game_url)
external_id=$(output rps_external_id)
...
Already registered? Update the existing row instead, or matchmaker will keep calling the old url:
    ...
    UPDATE game SET url = '$create_game_url', external_id = '$external_id'
     WHERE name = 'Rock-paper-scissors';
EOF
fi
