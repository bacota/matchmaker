...
object Protocol {
    ...
    /** Step 2. `next` is who may move now; empty on the move that ends the game.
      ...
      *
      * Here `startedAt` is the move before this one, or the match's creation for the opening move. That is what
      * matchmaker would have guessed — but a guess is only right for a game of alternating turns, and `engines/rps` is
      * not one.
      */
    ...
    /** Step 3. `scores` is an open map — matchmaker stores whatever the game puts there. This engine reports `outcome`
      * (win/loss/draw) and `moves` (how many marks the seat placed).
      */
