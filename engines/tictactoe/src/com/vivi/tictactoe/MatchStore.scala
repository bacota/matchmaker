...
trait MatchStore {
    ...
    /** Saves a new match. Fails if one with this id is already stored, since matchmaker generates a fresh match id per
      * game and a collision would mean two matches sharing a board.
      */
    ...
    /** Applies `f` to the stored match, saving whatever match it returns and answering with whatever else it returns.
      ...
      * winner's state is what makes the loser's move land on the board it will actually be played on rather than on a
...
object MatchStore {
    ...
    /** How many times a losing writer re-reads and re-applies before giving up. A tic-tac-toe match has two players and
      * nine cells; needing more than this means something other than contention.
      */
    val maxAttempts = 5
}
...
/** Matches in a DynamoDB table keyed by `matchId`, with the whole match as one JSON attribute.
  *
  * Nothing here queries by anything but the match id, so modelling the board as attributes would buy nothing and would
