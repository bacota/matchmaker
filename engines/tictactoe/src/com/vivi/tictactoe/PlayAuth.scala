...
/** Establishes which Cognito user is making a play request.
  *
  * The seat a player may move in is their own, and what says whose seat is theirs is the `cognitoId` matchmaker sent
...
trait PlayAuth {
    ...
    /** Whether the browser should be offered a sign-in. False for [[PlayAuth.Trusted]], where there is no login to
      * start and a button pointing at one would be a dead end.
      */
...
object PlayAuth {
    ...
    /** Deployed. The claims API Gateway's JWT authorizer already verified — signature, expiry, issuer and audience — so
      * this only reads `sub`.
      *
      * A request with no claims is not admitted: it means no authorizer ran, which is a route misconfigured rather than
      * a player to trust.
      */
    ...
    /** Local, against a real user pool. Verifies the bearer token itself — see [[JwtVerifier]].
      *
      * This is what makes a local engine exercise the deployed login flow rather than an imitation of it: the page
      * signs in against the real hosted UI, and the token it comes back with is checked against the real keys.
      */
    ...
    /** Local, with no user pool configured at all: whoever the caller says they are.
      *
      * The zero-setup mode, and the counterpart of matchmaker's `AUTH_MODE=header` — an engine pointed at a local
      * matchmaker in that mode has no tokens to verify anyway. Safe only because nothing that runs this way is
      * reachable from anywhere; the local server says so at startup.
      */
