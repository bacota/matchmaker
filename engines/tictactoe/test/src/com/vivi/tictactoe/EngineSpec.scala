...
class EngineSpec extends FunSuite {
    ...
    /** Who plays this mark — a move is authorized by the player's Cognito subject, which is what matchmaker sent as the
      * seat's `cognitoId`.
      */
    ...
    test("a game for anything but two players is refused") {
        ...
        assertEquals(
          ...
          Left(Refusal.Invalid("tic-tac-toe is a two-player game; 1 player(s) were sent"))
        )
    }
    ...
    test("a win ends the match, and matchmaker gets the move and then the results") {
        ...
        List((x, 0), (o, 3), (x, 1), (o, 4), (x, 2)).foreach((token, cell) =>
            assert(engine.move("m-1", token, cell).isRight, s"cell $cell")
        )
