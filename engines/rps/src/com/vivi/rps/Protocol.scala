...
object Protocol {
  ...
  /** Step 2. `next` is who may move now; empty on the move that ends the game.
    *
    * `takenAt` is when this move was made, which is when the clock starts for anyone in `next`;
    * `startedAt` is when the mover's own clock started for it, so the pair is what the move cost
    * them. Matchmaker takes both as stated rather than inferring either: it derives only the
