...
/** One player's seat in a match.
  *
  * `cognitoId` is who may throw in it — the same subject the player signs in as, which is how
...
/** One throw that was made: by whom, what it was, when it was made, and when that player's clock
  * started for it.
  *
  * A throw is this game's whole turn, so this is both the move and the turn record. `startedAt`
  * is the match's creation for both seats rather than the move before, because nobody here waits
  * for anybody: both clocks start when the match does.
  */
...
/** A match in progress, and everything needed to answer for it or to call matchmaker back.
  *
  * The state is just the two seats and whatever has been thrown into it. There is no turn to
  * store: whose turn it is, in a game where both players move at once, is "everyone who has not
  * moved" — so it is derived from `throws` rather than kept beside them, where the two could
  * disagree.
  *
...
case class RpsMatch(
  ...
  /** The seats still being waited on — all of them at the start, and none once the match is over.
    *
    * This is the whole of "whose turn it is" here, and it is a list because it is genuinely more
    * than one: matchmaker's move callback and status response both take several pending seats,
    * and this is the game that uses them.
    */
  def pending: List[Seat] = if (isOver) Nil else seats.filterNot(hasThrown)

  /** Over the moment both players have thrown, which is the rule the game is: nobody waits for a
    * turn, so there is nothing to end but the pair being complete.
    */
  def isOver: Boolean = seats.forall(hasThrown)

  /** The winning seat, once there is one. `None` while a throw is still to come, and `None` for a
    * draw — which [[isDraw]] is how to tell apart from an unfinished match.
    */
...
object RpsMatch {
  ...
  /** Seats the players, honouring the roles matchmaker sent when it sent usable ones.
    *
    * A game configured in matchmaker with roles named `One` and `Two` gets exactly those seats.
    * With no roles, or roles this engine does not recognise, the first player named takes One —
    * the engine still has to produce a playable game, and refusing would make role configuration
    * a prerequisite for trying it out. It costs nothing to be relaxed about it here: unlike X and
    * O, neither side of this game has any advantage over the other.
    */
