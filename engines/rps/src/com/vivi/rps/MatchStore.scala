...
trait MatchStore {
    ...
    /** Applies `f` to the stored match, saving whatever match it returns and answering with whatever else it returns.
      ...
      *
      * `None` overall means no such match. `f` may run more than once: a move is read-decide-write, and two players of
      * the same match can be mid-move at once — in Lambda even in two different containers. Retrying against the
      * winner's state is what makes the loser's move land on the state it will actually be played on rather than on a
...
object MatchStore {
    ...
    /** How many times a losing writer re-reads and re-applies before giving up. A match here is two players throwing
      * once each, and they may well throw at the same instant — which is the one contention this engine really expects.
      * Needing more than this means something else.
      */
    val maxAttempts = 5
}
...
/** Matches in a DynamoDB table keyed by `matchId`, with the whole match as one JSON attribute.
  *
  * Nothing here queries by anything but the match id, so modelling the throws as attributes would buy nothing and would
...
class DynamoDbMatchStore(http: SignedHttp, table: String, region: String) extends MatchStore {
    ...
    private def save(m: RpsMatch, expected: Option[Long]): Unit = {
        ...
        val condition = expected match {
            case Some(v) =>
                ujson.Obj(
                  ...
                  "ExpressionAttributeValues" -> ujson.Obj(":v" -> ujson.Obj("N" -> v.toString))
                )
        ...
        try call("PutItem", payload)
            ...
            case e: AwsError if e.getMessage.contains("ConditionalCheckFailedException") =>
                throw ConcurrentModification(m.matchId)
        }
    }
