...
/** The engine's HTTP surface, as a function from request to response.
  ...
  *
  * Three kinds of route, authorized three ways. The create and status calls are matchmaker's, and carry the API key
  * this engine and matchmaker share — checked here, since nothing in front of the function checks it. The play routes
  * are a player's, and carry a Cognito ID token from the same user pool matchmaker signs its players in with —
  * [[PlayAuth]] turns that into a subject and [[Engine.seatOf]] turns the subject into a seat. The board is nobody's
...
class Routes(engine: Engine, playAuth: PlayAuth, matchmakerKey: Option[String]) {
    ...
    /** Matchmaker, or not.
      ...
      * the engine is driven by curl and there is no secret to share. A deployed engine is never in that state: `Config`
      * refuses to start one without a key, so an unset variable fails at the first cold start rather than quietly
      * serving game creation to anyone who finds the url.
      *
    ...
    private def route(request: EngineRequest): EngineResponse =
        (request.method.toUpperCase, request.segments) match {
             ...
             * discloses nothing but the fact that a match id exists — no throw least of all. The alternative, refusing it, would
                ...
                parse[Protocol.MoveRequest](request.body).flatMap(move =>
                    ...
                    case Right(shape) =>
                        playAuth.callerOf(request) match {
                            case Left(refusal) => error(refusal)
                            case Right(caller) =>
                                engine.move(matchId, caller, shape) match {
                                    case Left(refusal) => error(refusal)
                                    case Right(applied) =>
            ...
            // no sight of either throw until the match has resolved — see `Engine.stateOf`.
            case ("GET", "matches" :: matchId :: "board" :: Nil) =>
                withPublic(matchId)(m =>
                    EngineResponse(
                      200,
                      Html.board(matchId, Some(engine.stateOf(m, None)), None, publicView = true),
    ...
    private def withSeat(request: EngineRequest, matchId: String)(
        f: (RpsMatch, Seat) => EngineResponse
    ): EngineResponse = {
        val answer =
            for {
    ...
    private def withPublic(matchId: String)(f: RpsMatch => EngineResponse): EngineResponse =
        engine.read(matchId) match {
            case Left(refusal) => error(refusal)
