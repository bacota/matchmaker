...
/** Verifies a Cognito ID token against the pool's published keys.
  *
  * Deployed, nothing uses this: API Gateway's JWT authorizer checks the signature, expiry, issuer and audience before
  * the function is invoked, and re-doing that per request would mean fetching JWKS on the request path for no gain. It
  * exists for the local server, where there is no gateway — and it verifies genuinely rather than emulating, because
  * the pool's JWKS endpoint is public: a process on localhost with no AWS credentials can validate real tokens from the
  * real user pool, which is what lets the same sign-in flow be exercised locally.
  *
  ...
  *   - `token_use` is `id`, since an access token from the same pool carries `client_id` rather than `aud` and would
  *     otherwise pass the audience check vacuously
...
class JwtVerifier(
    ...
    /** The `sub` of a valid token, or why it was refused. The message is deliberately the same shape for every failure
      * — a caller who is not signed in learns nothing about the token from which check rejected it.
      */
    def verify(token: String): Either[String, String] =
        try {
            ...
            if (parts.length != 3) Left("malformed token")
                ...
                for {
                    _ <- Either.cond(
                      header.obj.get("alg").map(_.str).contains("RS256"),
                      (),
                    ...
                    _ <- Either.cond(
                      claims.obj.get("iss").map(_.str).contains(issuer),
                      (),
                    ...
                    _ <- Either.cond(
                      claims.obj.get("exp").map(_.num).exists(exp => now().minus(skew).getEpochSecond < exp.toLong),
                      (),
                      "token has expired"
                    )
    ...
    private[rps] def parseJwks(body: String): Map[String, java.security.PublicKey] = {
