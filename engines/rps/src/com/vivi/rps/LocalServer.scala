...
/** Runs the engine on a local port, with matches in memory and no AWS involved.
  ...
  * GAME_EXTERNAL_ID=rps-dev mill -j 4 engines.rps.runMain com.vivi.rps.LocalServer
  *
  * # or with nothing to call back to, to just play the board:
  * MATCHMAKER_OFFLINE=true mill -j 4 engines.rps.runMain com.vivi.rps.LocalServer
  * }}}
  */
object LocalServer {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val port = sys.env.get("PORT").flatMap(_.toIntOption).getOrElse(8091)
    ...
    val routes = Config.routes(
      ...
      announce = m => {
        ...
        m.seats.foreach(seat => println(s"  ${seat.side} ${seat.cognitoId} (participant ${seat.participantId})"))
    ...
    println(s"rock-paper-scissors engine listening on http://localhost:$port")
