...
/** Lambda entry point, behind an API Gateway HTTP API using payload format 2.0.
  *
  * Handler string: `com.vivi.rps.Handler::handleRequest`.
  *
  * The engine is built once per container, as matchmaker's handler builds its services: nothing in it is per-request,
  * and the DynamoDB store's signer and HTTP client are worth keeping warm.
  */
