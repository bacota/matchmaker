...
/** What a successful throw produced, for the caller to answer with and for the callbacks below.
  *
  * `finished` means this throw was the second one, so the match resolved on it — which is the only way a match here
  * ever ends. `turn` is the throw itself, which the move callback reports: what it was, when it was made, and when this
  * player's clock started for it.
  */
...
class Engine(
    ...
    /** Step 1: create a game. The urls handed back are where matchmaker checks status, where the players play, and —
      ...
      * not a seat that leaks — which matters more here than in a game played on an open board, since a seat is also the
      * right to see a throw.
      */
    ...
    /** Step 4's other half: what matchmaker asks for when a participant hits refresh.
      *
      * Every seat that has not thrown is pending, which at the start of a match is both of them — this is the shape of
      * status that a turn-taking game never produces, and matchmaker records it as two seats on the clock at once.
      *
      * `prevMoveAt` is when the seat's clock started, and here that is the match's creation for both seats however late
      * either of them throws. Nobody is waiting for a predecessor: a player who takes an hour to decide spends an hour
      * of their own budget, and the other player's deadline is not moved by it.
      *
      * `since` is the last turn matchmaker has recorded; the throws made after it come back in `turns`. That is how a
      * chess-clock limit is charged, and it is also how a move callback that was lost is recovered as more than a
      * corrected deadline.
      */
    ...
    /** A player's throw. Decides against the stored match — atomically, so that two players throwing at the same moment
      * cannot both be recorded as the first — and then, having committed, calls matchmaker.
      *
    ...
    /** Steps 2 and 3, in that order: every throw is reported, and the throw that resolves the match is followed by the
      * results.
      *
      * `next` is always empty, including on the first throw of the two. The seat still to throw has been pending since
