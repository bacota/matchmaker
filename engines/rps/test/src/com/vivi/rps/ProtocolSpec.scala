...
/** The one place the engine and matchmaker are compared directly.
  ...
  *
  * What this engine adds to tic-tac-toe's version of the same suite is the simultaneous case:
  * two pending seats in one status answer, and a move callback that names nobody as next. Those
  * are shapes of the protocol matchmaker has always allowed and nothing exercised.
  *
...
class ProtocolSpec extends FunSuite {
  ...
  test("the engine's create response reads as matchmaker's") {
    val response = Protocol.CreateGameResponse("http://engine/status", "http://engine/play", Some("http://engine/board"))
