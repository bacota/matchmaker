...
/** Plays whole matches through the engine, checking both what a player is told and what
  ...
  * actually depends on.
  *
  * Most of what is checked here is the thing tic-tac-toe cannot check: that neither player waits
  * for the other, that the throws stay hidden until both are in, and that the match resolves on
  * whichever throw happens to be second.
  */
class EngineSpec extends FunSuite {
  ...
  test("someone with no seat in the match may not throw in it") {
    val (engine, recorder, _, _, _) = fixture()
    assertEquals(engine.move("m-1", "sub-carol", Shape.Rock), Left(Refusal.NotYours("'sub-carol' has no seat in match 'm-1'")))
