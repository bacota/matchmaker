...
/** Verification against real keys and real signatures — a token minted here with an RSA key, and the matching JWKS
  * served over HTTP the way Cognito serves its own.
  *
  * Signing tokens in the test rather than pasting a captured one is what lets the failure cases exist at all: an
  * expired token, one for another pool, one signed by the wrong key.
  */
class JwtVerifierSpec extends FunSuite {
          ...
          ujson
              .write(
                ujson.Obj(
                  ...
                  "token_use" -> tokenUse,
                  "sub" -> sub,
                  "exp" -> expiresAt.getEpochSecond.toDouble
                )
              )
              .getBytes(UTF_8)
        )
    ...
    /** Serves one JWKS document on a loopback port, so the verifier fetches over HTTP as it will in production rather
      * than being handed keys directly.
      */
    ...
    test("a token from another issuer is refused") {
        withJwks(jwks("key-1", keys)) { verifier =>
            assertEquals(
              verifier.verify(token(iss = "https://cognito-idp.us-east-1.amazonaws.com/somewhere-else")),
              Left("token is from another issuer")
            )
        }
    }
    ...
    test("a token naming a key the pool does not publish is refused") {
        withJwks(jwks("key-1", keys)) { verifier =>
            assertEquals(
              verifier.verify(token(kid = "key-2", iss = issuerOf(verifier))),
              Left("token names an unknown key")
            )
        }
    }
    ...
    test("alg=none and other malformed tokens are refused rather than accepted") {
        withJwks(jwks("key-1", keys)) { verifier =>
            assertEquals(
              verifier.verify(token(iss = issuerOf(verifier), alg = "none")),
              Left("unsupported token algorithm")
            )
    ...
    test("a multi-valued aud claim is accepted when this application is among them") {
        withJwks(jwks("key-1", keys)) { verifier =>
            assertEquals(
              verifier.verify(
                token(iss = issuerOf(verifier), aud = ujson.Arr(ujson.Str("other"), ujson.Str(audience)))
              ),
              Right("sub-alice")
            )
        }
