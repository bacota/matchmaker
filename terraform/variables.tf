...
variable "engine_api_keys" {
  description = <<-EOT
    Game engines that may post callbacks, as `external_id = key` — for engines run by someone
    else, whose key is agreed out of band.

    The engine bundled with this repository is not listed here: its key is generated by the root
    module and merged in, so that nobody has to keep two copies of one secret in step.
  EOT
  type        = map(string)
  default     = {}
  sensitive   = true
...
variable "game_engine_api_keys" {
  description = <<-EOT
    The other direction, as `host = key`: the key matchmaker presents when it calls a game engine
    someone else runs. Same remark about the bundled engine.
  EOT
  type        = map(string)
  default     = {}
  sensitive   = true
