...
locals {
  ...
  # A player's own routes: the state they see and the throw they make. Behind the same Cognito
  # user pool matchmaker signs its players in with, because the seat a player may move in is
  # found by the `sub` of their token — and here a seat is also the right to see a throw, since
  # the state route withholds the opponent's until both are in.
  player_routes = [
  ...
  # a player has thrown, never what, until the match has resolved.
  open_routes = [
 ...
 *
 * That compare-and-set matters more here than in tic-tac-toe: both players are on the clock at
 * once, so two throws arriving together is the ordinary case rather than a rare race, and a lost
 * one would be a throw a player believes they made.
 */
