...
variable "lambda_jar_path" {
  description = "Path to the assembled engine jar (`mill -j 4 --ticker false engines.rps.assembly`)."
  type        = string
}
...
variable "lambda_timeout_s" {
  description = "Lambda timeout. A throw is two DynamoDB calls and up to two callbacks to matchmaker."
  type        = number
  default     = 15
}
...
variable "point_in_time_recovery" {
  description = "Continuous backups for the match table. Off by default: a match here is two throws and is not worth restoring."
  type        = bool
  default     = false
}
